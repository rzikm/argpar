...
private:
	...
	{
		std::unique_ptr<argpar::option> option = std::make_unique<argpar::option>(aliases, hint, dest);
	...
	void verify_options()
	{
		for (auto && option : options_)
		{
			if (option->mandatory() && !option->found())
				throw argpar::missing_option(option->short_name() ? make_str(option->short_name()) : option->long_name());
		}
	}

	void add_aliases(argpar::option * opt)
	{
		// check for duplicate aliases
		if (opt->short_name())
		{
			auto[_, inserted] = short_to_option_.try_emplace(opt->short_name(), opt);
			if (!inserted) throw std::invalid_argument(make_str("Duplicate alias definition: ", opt->short_name(), "."));
		}
		if (!opt->long_name().empty())
		{
			auto[_, inserted] = long_to_option_.try_emplace(opt->long_name(), opt);
			if (!inserted) throw std::invalid_argument(make_str("Duplicate alias definition: ", opt->long_name(), "."));
		}
	}
